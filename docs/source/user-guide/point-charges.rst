...
The first term in Eq. :eq:`eq6` scales as :math:`1/R^{2}`
...
To compute the Liénard-Wiechert potentials and fields numerically, PyCharge solves the implicit relation in 
Eq. :eq:`eq3` for each charge and observation point to determine :math:`t_r`. This root-finding problem is
implemented efficiently using the `Optimistix <https://docs.kidger.site/optimistix/>`_
library, a JAX-native optimization framework that provides differentiable solvers.
...
to calculate the potentials and fields, are computed using JAX's automatic differentiation engine 
through :meth:`jax.jacobian`.
