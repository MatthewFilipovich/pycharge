...
.. code-block:: python
   ...
   sim_fn = jax.jit(simulate([dipole1, dipole2]))
   # coupled_states = sim_fn(ts)
...
.. note::
   ...
   While PyCharge computes the dynamics of the system, extracting quantitative parameters like the modified decay rate :math:`\gamma^{\pm}` and frequency shift :math:`\delta_{12}` requires post-processing. You can perform curve fitting on the simulation output (such as the kinetic energy over time) using functions from libraries like `SciPy <https://scipy.org/>`_ (e.g., ``scipy.optimize.curve_fit``).